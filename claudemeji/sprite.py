"""
sprite.py - individual-frame loader and animation player
...
Resolution order: context > posture > self
"""
...
class ActionDef:
    ...
    Compound state resolution via .resolve(posture, context):
    ...
    def __init__(
        ...
        **_kwargs,                    # absorb unknown keys from old configs gracefully
    ):
        self.files = files or []
        self.fps = fps
        self.loop = loop
    ...
    def resolve(self, posture: str = "standing", context: str | None = None) -> ActionDef:
        """Return the best ActionDef for the given state.
        Order: context > posture > self, then A/B variant selection."""
...
class SpritePlayer(QWidget):
    ...
    def play(self, action_name: str, posture: str = "standing", context: str | None = None,
        ...
        resolved = base.resolve(posture, context)
